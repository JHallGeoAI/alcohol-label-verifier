...
def should_run_rescue(
    ...
    ocr_text: str,
    score_threshold: float = 88.0,
) -> bool:
    """
    Run additional OCR passes only when the fast pass is incomplete
    or uncertain.
    """

    # Very little extracted text usually indicates an OCR failure.
    ...
    for result in results:
        ...
        # The government warning is always checked.
        if field == "Government warning":
            should_check_field = True
        else:
            expected_key = FIELD_EXPECTED_KEYS.get(field)
            should_check_field = bool(
                expected_key
                and str(expected.get(expected_key, "")).strip()
            )

        # Do not trigger rescue for optional fields left blank by the user.
...
def process_one(file, expected: Dict[str, str], rescue: bool = True) -> Dict:
    """Run a fast OCR pass first and use rescue OCR only when needed."""
