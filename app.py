...
st.caption("Decision-support prototype with batch verification, CSV reports, image rescue OCR, confidence scoring, and an exception queue.")
...
def result_value(result, attribute: str):
    """Support either dataclass-style or dictionary-style result objects."""
...
def should_run_rescue(
    ...
    """Run rescue OCR only for genuinely weak first-pass results."""

    # A very short OCR result usually means the fast pass failed outright.
    if len(ocr_text.strip()) < 120:
        return True

    for result in results:
        field = result_value(result, "field")
        ...
        # Warning text often receives a Review result for minor OCR variation.
        # Escalate only when the first pass is clearly poor.
        if field == "Government warning":
            if status == "Fail" and (numeric_score is None or numeric_score < 65):
                return True
            continue

        expected_key = FIELD_EXPECTED_KEYS.get(field)
        if not expected_key:
            continue

        # Skip optional fields that the user did not supply.
        if not str(expected.get(expected_key, "")).strip():
            continue

        # Hard failures on required fields justify the slower rescue path.
        if status == "Fail":
            return True

        # Reviews trigger rescue only when the score is materially weak.
        if status == "Review" and numeric_score is not None and numeric_score < 72:
            return True

    return False


def process_one(file, expected: Dict[str, str], rescue: bool = True) -> Dict:
    """Run fast OCR first and reserve rescue OCR for weak results."""
    total_start = time.perf_counter()
    file_bytes = file.getvalue()

    fast_start = time.perf_counter()
    fast_ocr = run_ocr(file_bytes, file.name, rescue=False)
    fast_results = run_verification(expected, fast_ocr.text)
    ...
    if rescue and should_run_rescue(
        results=fast_results,
        expected=expected,
        ocr_text=fast_ocr.text,
    ):
        rescue_start = time.perf_counter()
        rescue_ocr = run_ocr(file_bytes, file.name, rescue=True)
        rescue_results = run_verification(expected, rescue_ocr.text)
        rescue_seconds = time.perf_counter() - rescue_start

        final_ocr = rescue_ocr
        final_results = rescue_results
        rescue_used = True
    ...
    report = build_report(
        ...
        final_results,
        elapsed,
        quality=final_ocr.quality,
    )
    report["ocr_mode"] = "Image rescue" if rescue_used else "Fast pass"
    report["fast_pass_seconds"] = round(fast_seconds, 3)
    report["rescue_seconds"] = round(rescue_seconds, 3)
    return report
...
def render_single_result(report: Dict) -> None:
    ...
    fast_seconds = report.get("fast_pass_seconds", 0)
    rescue_seconds = report.get("rescue_seconds", 0)
    st.caption(
        f"OCR mode: {mode} | Fast pass: {fast_seconds:.2f} sec"
        + (f" | Rescue: {rescue_seconds:.2f} sec" if rescue_seconds else "")
    )
...
with tab_single:
    ...
    with left:
        ...
        st.text_input("Country of origin, if imported", value="", key="country_of_origin")
            ...
            "Automatically rescue uncertain images",
            value=True,
            key="single_rescue",
            help=(
                "Starts with fast OCR and runs additional preprocessing only "
                "when the initial result is incomplete or uncertain."
            ),
        )
...
with tab_batch:
    ...
    rescue_batch = st.checkbox(
        "Automatically rescue uncertain batch images",
        value=True,
        key="batch_rescue",
        help=(
            "Clean labels use the fast OCR path. Additional OCR passes "
            "run only for uncertain labels."
        ),
    )
        ...
        if st.button("Run batch verification", type="primary", use_container_width=True):
            ...
            for idx, row in expected_df.iterrows():
                ...
                if file is None:
                    ...
                    reports.append(process_one(file, expected, rescue=rescue_batch))
